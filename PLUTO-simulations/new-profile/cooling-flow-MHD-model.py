...
Run dirty-check.py before this to create the corresponding data to fit with the model
...
#v0byc0 = 0.049; T0 = 3.8e5; d0 = 7.9e-4*mu*mp ; c0 = np.sqrt(gamma*kB*T0/mu/mp) #orig fig in paper
v0byc0 = 0.05; T0 = 3.331e5; d0 = 2.982e-4*mu*mp ; c0 = np.sqrt(gamma*kB*T0/mu/mp) #5kpc
shift = 1.0
...
#inner solution
#Rin = 0.5 #starting of the inner radius
Rin = 0.8 #starting of the inner radius
...
axs[0, 0].grid()
#axs[0, 0].set_ylim(ymin=-3.7, ymax=-1.0)
axs[0, 0].set_ylim(ymin=-4.8, ymax=-1.0)
...
axs[0, 1].grid()
#axs[0, 1].set_ylim(ymin=2.41, ymax=2.53 )
axs[0, 1].set_ylim(ymin=1.87, ymax=2.13 )
...
#axs[0, 2].set_ylim(ymin=-10.3, ymax=10)
axs[0, 2].set_ylim(ymin=-13, ymax=15)
...
mult = 1e32
axs[1, 0].plot(Rcgs/kpc, np.log10(entcgs/mult), 'tab:red', linewidth=5)
...
axs[1, 0].set_ylabel(r'$\rm p_{gas}/\rho ^{\gamma}\ [\rm CGS \times 10^{%d}\ (log)$]'%fexp(mult), size=20 )
...
#axs[1, 1].set_ylim(ymin=3.9, ymax=6.2)
axs[1, 1].set_ylim(ymin=3.9, ymax=6.7)
...
#mult = 1e3
mult = 1e4
axs[1, 2].plot(Rcgs/kpc, Mdot/(Msun/yr)*mult, 'tab:purple', linewidth=5)
axs[1, 2].plot(radius*pc/kpc, Mdot_50*mult, c='tab:gray', linewidth=5)
axs[1, 2].fill_between(radius*pc/kpc, Mdot_16*mult, Mdot_84*mult, color='tab:gray', alpha=0.3)
axs[1, 2].grid()
axs[1, 2].set_xlim(xmin=0., xmax=rightedge)
axs[1, 2].set_ylim(ymin=-4.2, ymax = 6.3)
#axs[1, 2].set_ylabel(r'$\rm \dot{M}$ [$\rm \times 10^{-4} M_\odot yr^{-1}$] ', size=20 )
axs[1, 2].set_ylabel(r'$\rm \dot{M}$ [$\rm \times 10^{-%d} M_\odot yr^{-1}$] '%fexp(mult), size=20 )
...
plt.savefig('%s/plots/%s.png'%(base_dir,'pde_compare-marchinout-non-const'), transparent=True, bbox_inches='tight')
plt.show()
